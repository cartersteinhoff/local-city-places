...
export default function AdminInvitesPage() {
  ...
  const [filter, setFilter] = useState<"all" | "pending" | "used" | "expired">(
    "pending",
  );
  ...
  const handleDelete = async () => {
    ...
    try {
      const res = await fetch(
        `/api/admin/merchant-invites/${selectedInvite.id}`,
        {
          method: "DELETE",
        },
      );
      ...
      {loading ? (
        ...
        <>
          ...
          <div className="flex gap-2 mb-6">
            ...
            >
              <RefreshCw
                className={cn("w-4 h-4", isLoadingInvites && "animate-spin")}
              />
            </Button>
          </div>
                ...
                invites.map((invite) => (
                  <div key={invite.id} className="p-4">
                    ...
                    <div className="text-sm text-muted-foreground mb-2">
                      ...
                      {invite.usedAt && (
                        <p>Used: {formatDate(invite.usedAt)}</p>
                      )}
                    </div>
                    {invite.status === "pending" && (
            ...
            <DialogContent>
              <DialogHeader>
                ...
                <DialogDescription>
                  ...
                  {selectedInvite?.email ? ` for ${selectedInvite.email}` : ""}?
